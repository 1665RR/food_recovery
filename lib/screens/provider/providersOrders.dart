...
class _ProductOrdersScreenState extends State<ProductOrdersScreen> {
  ...
  Future _fetchProductOrders() async {
    ...
    _orders =
        (await ApiService().fetchOrdersByProducts(sharedToken!, widget.id));
            ...
            itemBuilder: (context, index) {
              return _orders!.isEmpty
                  ...
                  : Card(
                      child: ListTile(
                        subtitle: Text(
                          _orders![index].user!.email!,
                        ),
                        title: Text(
                          _orders![index].user!.name,
                            ...
                            _orders![index].status
                                ...
                                : OutlinedButton(
                                    onPressed: () async {
                                      ...
                                      try {
                                        var req = await ApiService()
                                            .changeStatus(sharedToken!,
                                                _orders![index].id);
